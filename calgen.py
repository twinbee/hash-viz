...
def generate_event_rows(month, year):
    ...
    for week in month_calendar:
        row = '<tr>\n'
        for day in week:
            if day == 0:  # This represents an empty day (non-day)
                row += '\t<td class="empty"></td>\n'
            else:  # Regular day with a date
                row += f'''\t<td class="day">
                            <table class="inner" id="j{month}{str(day).zfill(2)}">
                                <tr>
                                    <td class="dom">{day}</td>
                                </tr>
                                <tr>
                                    <td class="event">
                                        <?php fillIn({month}, {day}, {year}); ?>
                                    </td>
                                </tr>
                            </table>
                        </td>\n'''
