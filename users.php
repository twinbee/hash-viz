<?php
// ============================================
// USERS CONFIGURATION FILE
// ============================================
// This file contains user credentials for edit.php
// Keep this file separate to avoid overwriting when updating edit.php
//
// To add a new user:
// 1. Use password.php to generate an MD5 hash
// 2. Add the username and hash to the array below
// ============================================

$USERS = array(
	'kitchen' => 'fdf7c86c3904f5d41c7b22e38acfd84e',
	'bdb' => '1828c25220d3606e0ef9b8b704f46a88',
	'FruityPebbles' => 'b12cbb0934293c4ade0c4114b8b602da',
	'MBennett' => 'b599f1365e1832f5e66c8e88f8fede78',
	'Fourplay' => '4e6175b953b7488a33cafe71db52c3ae',
	'doubledribble' => 'd884306a107cfc48ac34bbb3d0a61917', // 2025-12-01 13:04:57
    'aileenkaufman' => 'b4205f1bd222c74799dfe719202930b7', // 2025-12-05 10:19:37

);
?>